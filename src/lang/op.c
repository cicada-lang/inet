...
get_free_port_op_t *
get_free_port_op_new(const node_spec_t *node_spec, port_index_t index) {
    get_free_port_op_t *self = allocate(sizeof(get_free_port_op_t));
    self->tag = GET_FREE_PORT_OP;
    self->node_spec = node_spec;
    self->index = index;
    return self;
}
...
void
get_free_port_op_destroy(get_free_port_op_t **self_pointer) {
    assert(self_pointer);
    if (*self_pointer) {
        get_free_port_op_t *self = *self_pointer;
        free(self);
        *self_pointer = NULL;
    }
}
